...
export const FeaturesGrid = () => {
  ...
  const { scrollYProgress: firstHemispherescrollY } = useScroll({
    target: firstHemisphereRef,
    offset: ["start end", "end start"],
  });

  const firstHemisphereTranslateY = useTransform(
    firstHemispherescrollY,
    [0, 1],
    [50, -50]
  );
  ...
  const { scrollYProgress: secondHemispherescrollY } = useScroll({
    target: secondHemisphereRef,
    offset: ["start end", "end start"],
  });

  const secondHemisphereTranslateY = useTransform(
    secondHemispherescrollY,
    [0, 1],
    [50, -50]
  );
  ...
  const { scrollYProgress: coneScrollProgressY } = useScroll({
    target: coneRef,
    offset: ["start end", "end start"],
  });
  const coneTranslateY = useTransform(coneScrollProgressY, [0, 1], [100, -100]);
  const coneRotate = useTransform(coneScrollProgressY, [0, 1], [12, 45]);
  return (
